...
export class SocketGateway {
    ...
    async handleVideoCall(client: Socket, data: { idChat: string, link: string }) {
        data.link && await firstValueFrom(this.natsClient.send({ cmd: 'redis_set' }, { key: `vc_${data.idChat}`, value: data.link }))
        !data.link && await firstValueFrom(this.natsClient.send({ cmd: 'redis_del' }, { key: `vc_${data.idChat}` }))
